		...
		<td>
			...
			<tr>
				...
				<td align="center">
					...
					<a href="javascript:del(<?=$cc1['build_id']?>)" class="b">Delete</a>				</td>
			</tr>
			<? }?>
			<tr>
		...
		<td>
			...
			<tr>
				...
				<td align="center">
					...
					<a href="javascript:del1(<?=$cc1['proj_id']?>)" class="b">Delete</a>				</td>
			</tr>
			<? }?>
			<tr>
				<td colspan="4" align="center" class="style8"><? paginate($start,$q_limit,$cccno,$filePath,"");?></td>
...
if($_GET['act'] == 'DLT')
{
	$del = mysql_query("DELETE FROM `real_builders` WHERE `build_id` = ".$_GET['id']."");
	//echo "delete from job-post where job_id = ".$_GET['id']."";
	echo '<script language="javascript">';
	echo 'window.location = "fetured.php?act=build"';	
	echo '</script>';
}
if($_GET['act'] == 'DLT1')
{
	$del = mysql_query("DELETE FROM `real_projects` WHERE `proj_id` = ".$_GET['id']."");
	//echo "delete from job-post where job_id = ".$_GET['id']."";
	echo '<script language="javascript">';
	echo 'window.location = "fetured.php?act=proj"';	
	echo '</script>';
}?>