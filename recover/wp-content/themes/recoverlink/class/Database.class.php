...
function num_rows($query_id) {
	$this->num_row = mysql_num_rows($query_id);
	return $this->num_row;
}
...
function fetch_all_array($query_id) {
	$out = array();
...
function query_select($table, $fields="", $whereand="", $whereor="", $order="", $group="", $limit="") {
 //collecting data from table
 //$table->'table name', $fields->'array of fields to be selected', $whereand->'array(field-value)', $whereor->'array(field-value)'
	$que = "SELECT ";
	if ($fields != "") {
		if (is_array($fields)) {
			reset($fields);
...
function query_delete($del, $where="") {
 //deletion of row
 //$del->'table name', $where->'array(field-value)'
	$que = "DELETE FROM $del";
	if ($where != "") {
		$que .= " WHERE ";
		if (is_array($where)) {
			$que .= implode(" AND ", $where);
		} else {
			$que .= "$where";
		}
	}
