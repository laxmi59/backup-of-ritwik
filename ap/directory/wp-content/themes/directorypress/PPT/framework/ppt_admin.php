...
add_filter('default_hidden_meta_boxes', array( $this, 'ppt_hidden_meta_boxes' ), 10, 2);
	...
	$this->change_post_object_label();
		...
		add_menu_page(basename(__FILE__), __(str_replace("Theme","",str_replace("EmployeePress","EmployeeP",str_replace("ComparisonPress","Comparison",constant('PREMIUMPRESS_SYSTEM')))),'cp'), "edit_pages", basename(__FILE__), '', ''.PPT_PATH.'/img/admin/'.strtolower(constant('PREMIUMPRESS_SYSTEM')).'.png',3); 
		...
		if(strtolower(constant('PREMIUMPRESS_SYSTEM')) != "comparisonpress"){  
		...
		add_submenu_page(basename(__FILE__), __(constant('PREMIUMPRESS_SYSTEM'),'cp'), __('<img src="'.PPT_PATH.'/img/admin/sgoogle.png" align="middle"> Google+','cp'), "edit_pages", 'analytics', array( $this,'premiumpress_admin_analytics'));
		...
		add_submenu_page(basename(__FILE__), __(constant('PREMIUMPRESS_SYSTEM'),'cp'),
		 __('<img src="'.PPT_PATH.'/img/admin/sorders.png" align="middle"> Order Manager','cp'), "edit_pages", 'orders', array( $this,'premiumpress_admin_orders'));	
	...
	fieldset.last {margin-bottom: 0; border-right: 1px solid #ccc !important;}
	fieldset legend {padding: 0 10px; font-size: 110%;}
	label,
	div.sep {display: block; margin-top: 6px;}
	label.check,
	label.radio {display: inline; margin-top: 0;}
	span.loading {margin-left: 10px;}
	ul.actions {margin: 0;}
