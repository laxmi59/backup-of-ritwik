...
print  '<img src="'.$PPT->ImageCheck(trim($_GET['d']),"full").'" style="max-width:600px;max-height:450px;">';
    ...
    foreach($result->Items->Item as $val){
	 ...
	 //die(print_r($val->CustomerReviews));
	...
	//	GET ATTRIBUTES
	if(isset($val->ItemAttributes->Brand)){	
		
		$extra_data = "<ul class=ExtraData>";
		foreach($val->ItemAttributes as $at1){
		
		foreach($at1 as $key => $att){
		...
		if(is_array($att[0])){
	 		...
	 		if(strlen($val) > 1){
				...
				$extra_data .= "<li><span>".$key."</span>:".$val. "</li>";
	...
	if($data['old_price'] < $data['price']){	 
	 ...
	 /* $time = current_time('mysql', $gmt = 0);	
	  ...
	  if(strtolower(constant('PREMIUMPRESS_SYSTEM')) == "comparisonpress" && get_option('comparisonpress_importamazon') == 1){ 
	  
	  $SAVEDPRODUCTID = $POSTID;
	  $key = get_option("google_productsearch_api");

$com = new ProductComparison($key);

$query = $com->getProduct($data['asin'])->orderBy("price","ascending")->language('us')->spellcheck()->pagination(20,1)->run(); // show 5 starting at page 1
// brand(array('canon'))->
 
// echo "total found: ".$query['total'];
foreach($query['products'] as $p){

	 // IMPORT PRODUCT COMPARE	 
	  $compare_product = array();
	  $compare_product['post_title'] 	= $p['product']['title'];
	  $compare_product['post_content'] 	= $p['product']['description'];	  
	  $compare_product['post_author'] 	= 1;
	  $compare_product['post_status'] 	= "publish";
	  $compare_product['post_category'] = explode(",",$_GET['catid']); 
	  $compare_product['post_type'] 	= "ppt_compare";
	  $POSTID = wp_insert_post( $compare_product );	 
	  
	  add_post_meta($POSTID, "linkedto", $SAVEDPRODUCTID);	
	  add_post_meta($POSTID, "image", $p['products']['images'][0]);	  
	  add_post_meta($POSTID, "link", $p['product']['link']);
 	  add_post_meta($POSTID, "price", $p['product']['price']);
	  add_post_meta($POSTID, "availability", $p['product']['availability']);
	  add_post_meta($POSTID, "currency", $p['product']['currency']);
	  add_post_meta($POSTID, "brand", $p['product']['brand']);
	  add_post_meta($POSTID, "condition", $p['product']['condition']);
 	  add_post_meta($POSTID, "retailer", $p['seller']['name']);
 	add_post_meta($POSTID, "country", $p['seller']['country']);
...
case "WishList":{  
	...
	if($_GET['act'] == "add"){
		...
		print "<div style='background:#009800;color:#fff;padding:5px;'>".getProductName($_GET['itemID'])." ".$GLOBALS['_LANG']['_wish1']." <a href='".get_option("wishlist_url")."' style='color:white;text-decoration:underline;'>".$GLOBALS['_LANG']['_wish2']."</a> </div>";
		
		}else{
		
		print "<div style='background:#bc1b00; color:#fff;padding:5px;'>".getProductName($_GET['itemID'])." ".$GLOBALS['_LANG']['_wish3']." <a href='".get_option("wishlist_url")."' style='color:white;text-decoration:underline;'>".$GLOBALS['_LANG']['_wish2']."</a> </div>";
