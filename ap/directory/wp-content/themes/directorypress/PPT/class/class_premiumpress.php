...
/* ------------------------------------------------------------------------------- */
	...
	function Action_Edit(){ 
		...
		if(get_option("display_listing_status") != ""){
			...
			// NEW SECTION ADDED IN 6.5, AUTO APPROVE FREE LISTINGS
	...
	function Action_Add(){  
		...
		if(isset($_POST['form']['trackingID'])){
			...
			add_post_meta($POSTID, 'reclink', $_POST['reclink']);
			}
...
/************ SHRINK THE WEB FUNCTIONALITY ******************/
	...
	function CheckPrune($post){
		...
		if($GLOBALS['premiumpress']['post_prun'] == ""){ $GLOBALS['premiumpress']['post_prun'] = get_option("post_prun"); }
		if($GLOBALS['premiumpress']['prun_period'] == ""){ $GLOBALS['premiumpress']['prun_period'] = get_option("prun_period"); }
		if($GLOBALS['premiumpress']['prun_status'] == ""){ $GLOBALS['premiumpress']['prun_status'] = get_option("prun_status"); }
...
class ProductComparison {
	...
	const API_BASE = 'https://www.googleapis.com/shopping/search/v1/public/products';
	
	private static $key = NULL;
	
	public static $lang = 'US';
	
	private static $parameters;
	
	private static $_orderby = array('relevancy','modificationTime','price');
	...
	{
		self::$parameters['q'] = $product;
		
		return $this;
	}
	
	public function orderBy($type, $order = 'ascending')
	{
		// only add parameter when the type could be found in $_orderby
		if(in_array($type, self::$_orderby) && in_array($order,array('ascending','descending')))
		{
			self::$parameters['rankBy'] = 'price:' . $order;
		}
	...
	{
		// retrieve args
		$args = (func_num_args() <= 1 &&  is_array($values)) ? $values : func_get_args();

		// set brand values
		$brand = 'brand:' . implode('|', $args);
		
		// also check if restrictBy already exists
		self::$parameters['restrictBy'] = 
			(isset(self::$parameters['restrictBy'])) ? 
			self::$parameters['restrictBy'] . ',' . $brand : 
			$brand;
	...
	{
		//used, refurbished, and new
	}
	
	public function spellcheck()
	{
		// enable spelling check 
		self::$parameters['spelling.enabled'] = 'true';
		
		return $this;
	}
	
	public function pagination($maxResults = 25, $startIndex = 1)
	{
		if(is_int($maxResults) && is_int($startIndex))
		{
			self::$parameters['startIndex'] = $startIndex;
			self::$parameters['maxResults'] = $maxResults;
		}
		return $this;
	}
	
	public function language( $country = 'us')
	{
		self::$parameters['country'] = $country;
	...
	{
		// additional checks
		if( self::$key === NULL || !isset(self::$parameters['q']))
		{
			return FALSE;
		}
		...
		// add country
		if( !isset(self::$parameters['country']))
		{
			self::$parameters['country'] = self::$lang;
		}
		
		$return = self::send( self::API_BASE . '?' . http_build_query(self::$parameters));
	...
	public static function send($url)
	{
		...
		// solves content length > 1025 bytes issue
		...
		self::$debug = curl_getinfo($ch);
	
		curl_close( $ch);
		
		if ( self::$debug['http_code'] === 200) {
			return json_decode( $content, TRUE);
		}
		else {
			die('ERROR: <pre>' . print_r( self::$debug, TRUE) . '</pre>');
		}
	...
	public static function format( $arr)
	{
		...
		$tmp['searchterm'] = htmlentities( self::$parameters['q']);
		
		$tmp['total'] = $arr['totalItems'];
		...
		{
			$tmp['products'][$i] = array(
				'google' => array(
					'name' => $arr[$i]['product']['author']['name'],
					'country' => $arr[$i]['product']['country'],
				),
				'seller' => array(
					'id' => $arr[$i]['product']['author']['accountId'],
					'name' => $arr[$i]['product']['author']['name'],
					'country' => $arr[$i]['product']['country'],
				),
				'product' => array(
					'id' => $arr[$i]['product']['googleId'],
					'title' => $arr[$i]['product']['title'],
					'brand' => $arr[$i]['product']['brand'],
					'description' => $arr[$i]['product']['description'],
					'condition' => $arr[$i]['product']['condition'],
					'link' => $arr[$i]['product']['link'],
					'availability' => $arr[$i]['product']['inventories'][0]['availability'],
					'price' => $arr[$i]['product']['inventories'][0]['price'],
					'currency' => $arr[$i]['product']['inventories'][0]['currency'],	
				),
			
			);
			// fix for multiple images
			foreach($arr[$i]['product']['images'] as $c => $url)
			$tmp['products'][$i]['products']['images'][] = $url['link'];
