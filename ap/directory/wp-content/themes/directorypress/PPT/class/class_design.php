...
function GALLERYBLOCKDO(){
    ...
    foreach ($postslist as $loopID => $post){ 
		...
		if( ( $post->post_type  =="article_type" || $post->post_type  =="faq_type" ) ){ 
	...
	if($package > 0){
		...
		if(PREMIUMPRESS_SYSTEM == "directorypress"){
		$pakSTRING 		.= "<br />Expires: ".$expires."</small>";
		}elseif(substr($expires,0,4) == date("Y")){
		$pakSTRING 		.= "<br />Expires: ".$expires."</small>";
		}
	}
			...
			if(get_option("PPT_slider_items") =="featured"){
			 ...
			 switch($sliderID){
				...
				case "2": {
						...
						$sortedSlider = $sliderData;//$this->array_sort($sliderData, 'order', SORT_ASC); &amp;w=695
						$i=0; foreach($sortedSlider as $slide){ 
						...
						$STRING .= '<li class="featured-itemImage">                
							...
							<img src="'.$PPT->Image($slide,$imgT,"").'" alt="'.$slide->post_title.'" />                 
								<span>                       
			...
			switch($sliderID){
				...
				case "2": {
						...
						$STRING .= '<li class="featured-itemImage">                
							...
							<img src="'.$PPT->ImageCheck($slide['s2'],$imgT,"").'" alt="'.$slide['s3'].'" />                 
								<span>                       
...
function LAY_NAVIGATION(){
		...
		if(isset($GLOBALS['premiumpress']['showpages']) && $GLOBALS['premiumpress']['showpages'] =="yes"){ 
...
/*************************************************************************************************/
	 ...
	 if($footer){
	 $showp = get_option("footer_excluded_pages");
	 if($showp ==""){ return $showp; }
	 $main_nav = get_pages("parent=0&sort_column=menu_order&include=".$showp);
	 }else{
