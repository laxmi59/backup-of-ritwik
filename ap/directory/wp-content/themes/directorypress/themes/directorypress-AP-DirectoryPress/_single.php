...
<?php echo $PPTDesign->GL_ALERT($GLOBALS['error_msg'],$GLOBALS['error_type']); ?>
    ...
    <div class="append border_r clearfix"> 
        ...
        <div class="thumbnail-large" style="margin-right:20px;"> 
            <div class="shadowJoomlaIndex"><div><div>
           <img src="<?php echo $PPT->Image($post->ID,"url","&amp;w=180&amp;h=128"); ?>" alt="<?php the_title(); ?>" /></div></div></div>
        ...
        <a href="<?php echo $link; ?>" target="_blank" title="<?php the_title(); ?>" <?php if($GLOBALS['premiumpress']['nofollow'] =="yes"){ ?>rel="nofollow"<?php } ?>>Join Now<?php //echo SPEC($GLOBALS['_LANG']['_single1']) ?></a>
        <?php } ?>
        <a href="<?php echo get_option('contact_url'); ?>?report=<?php echo $post->ID; ?>" title="report this listing"><?php if(is_single()) echo "Report Listing"; else echo SPEC($GLOBALS['_LANG']['_single2']) ?></a>       
        </div> 
