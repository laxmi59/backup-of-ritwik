...
/*************************************************************************************************/
	...
	if(isset($Maincategories)){
            ...
            if($Maincatcount > 0){ $catlist .= '<div class="homeCategories"><table><tr>';}
                $tdcount=0;$trcount=2;
                foreach ($Maincategories as $Maincat) { if(strlen($Maincat->name) > 1){ 
                    if(is_home() && $Maincat->parent ==0){		
                        $catlist .= '<td valign="top"><span><a href="'.get_category_link( $Maincat->term_id ).'" title="'.$Maincat->category_nicename.'" style="font-size:16px; color:#454444;"><b>';
			...
			if($SHOWCATCOUNT =="yes"){ $catlist .= " (".$Maincat->count.')</b></a></span>'; }else{ $catlist .= '</b></a></span>'; }
				...
				foreach ($categories as $cat) { //if($stopShow < 3){
                                    ...
                                    //$catlist .= '</a>, ';
                                    if($scatcount < count($categories)-1){
					$catlist .= '</a>, ';
                                    }else{
					$catlist .= '</a> ';
                                    }
			...
			//$catlist .= '</li>';
						...
						$catlist .= '<td><span><a href="'.get_category_link( $Maincat->term_id ).'" title="'.$Maincat->category_nicename.'" style="font-size:16px; color:#454444;"><b>';
						$catlist .= $Maincat->name;
						if($SHOWCATCOUNT =="yes"){ $catlist .= " (".$Maincat->count.')</b></a></span>'; }else{ $catlist .= '</b></a></span>'; }
						//$catlist .= '</span></a>';
			...
			if($Maincatcount > 0){ $catlist .= '</tr></table><div class="clearfix"></div></div>'; }
