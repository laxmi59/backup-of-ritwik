...
class EasyGoogleMap {
	 ...
	 * @return: void
	 */
	function SetMarkerIconStyle($style="STAR") {
	...
	function InitJs() {
		...
		if ($is_error) { 
			...
			for ($i=$cnt_add-1; $i>=0; $i--) {
				...
				$ret .= "		}\n";
				$ret .= "		else {\n";
				$ret .= "		  map.setCenter(new GLatLng(51.50015,-0.12624), {$this->mMapZoom});\n";
				$ret .= "		}\n";
				$ret .= "	  }\n";
	...
	function CheckConf() {
		...
		if (!array_key_exists($this->mDefStyle, $this->mIconStyle)) {
			$ret .= "<h1>INVALID setMarkerIconStyle(FLAG) PARAMETER: $this->mDefStyle</h1><br />\n";
			$ret .= "<b>POSSIBLE PARAMETER VALUES: </b><br />\n";
