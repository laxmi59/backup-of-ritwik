	...
	*/
	...
	class HighriseAPI
	{
		...
		protected function postDataWithVerb($path, $request_body, $verb = "POST")
		{
			...
			echo $url = "https://" . $this->account . ".highrisehq.com" . $path;
		...
		public function findPeopleBySubjectFields($id)
		{
			$url = "/people/search.xml?criteria[subject_field_id]=" . $id;
			$people = $this->parsePeopleListing($url);
			return $people;
		}
		//https://ritwik2.highrisehq.com/people/search.xml?criteria[subject_field_id]=647490
	...
	class HighrisePerson extends HighriseAPI
	{
		...
		public function loadFromXMLObject($xml_obj)
		{
			...
			//print_r($xml_obj->{'subject_datas'});exit;
			...
			$this->loadSubjectDatasFromXMLObject($xml_obj->{'subject_datas'});
		...
		public function loadSubjectDatasFromXMLObject($xml_obj)
		{
			//print_r($xml_obj);exit;
			if (isset($xml_obj->{'subject_datas'}))
			{
				foreach($xml_obj->{'subject_datas'}->{'subject_data'} as $value)
				{
					$subject_data = new HighriseSubjectData($value->{'id'}, $value->{'value'}, $value->{'subject_field_id'}, $value->{'subject_field_label'});
					$this->subject_datas[] = $subject_data;
					
				}
			}elseif(isset($xml_obj->{'subject_data'})){
		...
		public function addSubjectData($value, $subject_field_id, $subject_field_label)
		{
			$item = new HighriseSubjectData();
			$item->setValue($value);
			$item->setSubjectFieldId($subject_field_id);
			$item->setSubjectFieldLabel($subject_field_label);
				
			$this->subject_datas[] = $item;
		}
