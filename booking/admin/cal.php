...
function hiLightEvt($eMonth,$eDay,$eYear,$cat)
{
	...
	if($_GET['month']<10)
		//$sql123="SELECT * FROM ".SLOTS." where slot_persons<>0 and `slot_cid`=$_GET[ser_id] and `slot_date` = '" . $eMonth . '/' . $j . '/' . $eYear . "'";
	$sql123="select * from ".BOOK;
	$result=mysql_query($sql123);
	$res=mysql_num_rows($result);
	echo $res;
	while($row= mysql_fetch_array($result)){
	if($res >=1){
		$book=mysql_num_rows(mysql_query("select * from ".USERS." where `user_id`=$row[user_id]"));
		if($book<>''){
				$aClass = 'class="event"/'.$row['user_id'];
			}else{
				$aClass = 'class="normal"';
			}
		}elseif($res == 0){
...
?>
<table cellpadding="0" width="50%" align="center" id="table" cellspacing="0">
<tr>
...
for($i = 1; $i < $numDays+1; $i++, $counter++){
	...
	<? if($ttt1[1]==''){ echo $i;}else{ 
		if(strlen($i)==1){$j='0'.$i;}else{ $j=$i;}?>
			<a href="reg.php?act=all_times&ser_id=<?=$_GET['ser_id']?>&id=<?=$month."/".$j."/".$year?>" class="tt"><?=$i;?></a>
		<? }?>
	</td>
	<?php
}
