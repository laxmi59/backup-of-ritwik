...
<tr>
	...
	<tr>
		<td width="42%" class="style3">Details</td>
		<td width="58%" class="style3">Map</td>
	</tr>
	...
	<? 
	 if($_GET['cid']){
	  ...
	  <tr class="tabcolor">
	  	<td colspan="2" align="center">No Listings Found in <?=$_GET['cid']?><?=$_GET['aid']?></td>
	  </tr>
	 <? }else{?>
	 <tr class="tabcolor">
		...
		<? print $prop->property2($xx['list_property'])." for ".$req->req1($xx['list_type'])." in ".$xx['list_location'].",".$xx['list_city']//.","?><br />
		Area <?=$xx['list_area']?>Sq.Ft with <?=$xx['list_bedroom']?>Bedrooms<br />
		Rs.<?=$xx['list_price']?>,<b><? if($xx['list_pricing']=='yes'){ echo "Negotiable";}else{ echo "Not Negotiable";} ?></b>
		<?="Ref.No:".$xx['list_gid'];?>
		</td></tr>
		<tr>
		  <td class="tabcolor" align="right"  style="border-bottom:1px solid;">
